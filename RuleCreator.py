...
class RuleCreator(object):
    def __init__(self, datapath, nrclus,varnames,m=2,max_it=1000,error=0.005,seed=None,shape='gauss2',global_fit=1):
        self.data=np.loadtxt(datapath,delimiter=',')
        self.nrclus=nrclus
        self.varnames=varnames
        self.shape=shape
        self.xdata=self.data[:,0:-1]
        self.ydata=self.data[:,-1]
        
        _,self.partitionmatrix,_= self.fcm(self.xdata,nrclus,m,max_it,error,seed)
        self.MFs = self.DetermineMF(self.xdata,self.partitionmatrix,shape)
